# ROS2 CUDA 11.8 + Polymetis (libfranka 0.14.x) Docker 配置

## 概述

此目录包含用于同时运行 ROS2 Humble with CUDA 11.8 和 ROS2 Polymetis with libfranka 0.14.x 的组合 Docker Compose 配置。

这个配置启动两个容器：
- **ros2_cu118**: ROS2 Humble with CUDA 11.8 和 ZED SDK（用于相机支持）
- **ros2_polymetis**: ROS2 Polymetis with libfranka 0.14.x（用于机器人控制）
...
- **libfranka**: 0.14.1
- **Polymetis**: Compatible with robot system 5.7.1
- **C++ Standard**: C++14
...
```bash
cd /home/yjin/repos/role-ros2/docker/ros2_cu118_franka_0.14.x
