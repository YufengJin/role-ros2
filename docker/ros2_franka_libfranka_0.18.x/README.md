# ROS2 Foxy + Polymetis Docker 环境 - libfranka 0.18.x

此目录包含用于运行 ROS2 Foxy 和 Polymetis 机器人控制的 Docker 配置文件（libfranka 0.18.x 版本）。

## 特性

- **ROS2 Foxy** + FastRTPS (与官方镜像一致)
- **Polymetis** 机器人控制框架（源码构建）
- **libfranka 0.18.2** Franka 机器人支持
  - ✅ 使用 **C++17** 标准
  - ✅ 需要 **fmt 库**支持
  - ✅ 兼容机器人系统 **5.9.0+**
...
| 文件 | 说明 |
|------|------|
| `Dockerfile` | Docker 镜像定义（libfranka 0.18.x，C++17，fmt 库）|
...
**请根据您的机器人系统版本选择对应的目录：**
- 机器人系统 **5.9.0 或更高** → 使用 `ros2_franka_libfranka_0.18.x`（当前目录）✅
- 机器人系统 **5.7.1** → 使用 `ros2_franka_libfranka_0.14.x`
...
| `LIBFRANKA_VERSION` | `0.18.2` | Libfranka 版本（在构建时使用）|
...
└── polymetis/              # Polymetis 安装目录
    └── polymetis/
        ├── python/         # Python 模块
        └── build/          # C++ 构建产物（libfranka 0.18.x with C++17, fmt）
```
...
### 如何切换到 libfranka 0.14.x？

如果您需要 libfranka 0.14.x 版本（兼容机器人系统 5.7.1），请使用另一个目录：
...
- **libfranka**: 版本 0.18.2，使用 C++17，需要 fmt 库
- **Polymetis**: 使用 C++17（与 libfranka 0.18.x 匹配）
- **protobuf**: 版本 3.18.1（Python 和 C++ 一致）
- **gRPC**: 版本 1.41.1
- **fmt**: 系统库，通过 `libfmt-dev` 安装
...
# Polymetis 构建（与 libfranka 0.18.x 匹配）
cmake .. \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_CXX_STANDARD=17 \
    -DBUILD_FRANKA=ON \
