#!/usr/bin/env python3
"""
Test script for Joint Position Control mode with arm and gripper.

This script:
1. Switches to joint_position_example_controller
...
from controller_manager_msgs.srv import SwitchControllers, ListControllers
from franka_msgs.action import Move as GripperMove
from rclpy.action import ActionClient


class JointPositionControlTester(Node):
    def __init__(self):
        super().__init__('joint_position_control_tester')
        
        # Service clients
        self.switch_controllers_client = self.create_client(
            SwitchControllers, '/controller_manager/switch_controllers'
        )
    ...
    def run_test(self):
        """Run the joint position control test"""
        self.get_logger().info('=' * 60)
        self.get_logger().info('Starting Joint Position Control Test')
        self.get_logger().info('=' * 60)
        ...
        # Step 3: Activate joint_position_example_controller
        self.get_logger().info('\n[Step 3] Activating joint_position_example_controller...')
        ...
        if not success:
            self.get_logger().error('Failed to activate joint_position_example_controller')
        ...
        self.get_logger().info('\n[Step 4] Running joint position controller for 5 seconds...')
        self.get_logger().info('WARNING: Robot will move joints in periodic pattern!')
        ...
        self.get_logger().info('\n' + '=' * 60)
        self.get_logger().info('Joint Position Control Test Completed Successfully!')
        self.get_logger().info('=' * 60)
        return True
...
def main(args=None):
    ...
    tester = JointPositionControlTester()
    ...
    try:
        ...
        tester.switch_controllers([], ['joint_position_example_controller'], strictness=2)
        sys.exit(1)
