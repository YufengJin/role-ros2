...
if [ -f "/app/ros2_ws/src/role-ros2/docker/polymetis_ros2.env" ]; then
    # Docker environment - use system launch_gripper.py
    LAUNCH_GRIPPER_CMD="launch_gripper.py"
    PYTHON_ENV=""
else
    # Host system - use conda environment
    CONDA_ENV="/home/yjin/miniconda3/envs/polymetis-local"
    if [ -d "$CONDA_ENV" ]; then
        LAUNCH_GRIPPER_CMD="$CONDA_ENV/bin/launch_gripper.py"
...
# Check if port 50052 is still in use and kill the process using it
# First, try to find and kill the process
if command -v lsof >/dev/null 2>&1; then
    PORT_PID=$(lsof -ti :50052 2>/dev/null || true)
    if [ -n "$PORT_PID" ]; then
        echo "Killing process using port 50052 (PID: $PORT_PID)"
    ...
    if fuser 50052/tcp >/dev/null 2>&1; then
        echo "Killing processes using port 50052 with fuser"
        fuser -k 50052/tcp 2>/dev/null || true
        if command -v sudo >/dev/null 2>&1; then
            sudo fuser -k 50052/tcp 2>/dev/null || true
        fi
        sleep 1
    fi
fi
...
while [ $RETRY -lt $MAX_RETRIES ]; do
    ...
    if check_port_connectable 50052; then
        ...
        if command -v lsof >/dev/null 2>&1; then
            PORT_PID=$(lsof -ti :50052 2>/dev/null || true)
            if [ -n "$PORT_PID" ]; then
                echo "Port 50052 still connectable, killing PID $PORT_PID again..."
                kill -9 "$PORT_PID" 2>/dev/null || true
                if command -v sudo >/dev/null 2>&1; then
                    sudo kill -9 "$PORT_PID" 2>/dev/null || true
                fi
    ...
    if command -v lsof >/dev/null 2>&1; then
        if lsof -ti :50052 >/dev/null 2>&1; then
            PORT_IN_USE=true
        fi
    elif command -v netstat >/dev/null 2>&1; then
        if netstat -tuln 2>/dev/null | grep -q ":50052 "; then
            PORT_IN_USE=true
        fi
    elif command -v ss >/dev/null 2>&1; then
        if ss -tuln 2>/dev/null | grep -q ":50052 "; then
            PORT_IN_USE=true
        fi
    fi
    ...
    RETRY=$((RETRY + 1))
    echo "Port 50052 still in use, waiting... ($RETRY/$MAX_RETRIES)"
    sleep 2  # Wait 2 seconds between retries (TIME_WAIT typically lasts 60-120 seconds)
done

if [ "$PORT_FREE" = "false" ]; then
    echo "Warning: Port 50052 may still be in use after $MAX_RETRIES retries (${MAX_RETRIES}0 seconds)."
...
# Launch gripper server
# Note: For Robotiq gripper, uncomment and modify the line below:
# chmod a+rw /dev/ttyUSB0
# launch_gripper.py gripper=robotiq_2f gripper.comport=/dev/ttyUSB0

if [ -n "$PYTHON_ENV" ]; then
    # Host system: use explicit Python path
    "$PYTHON_ENV" "$LAUNCH_GRIPPER_CMD" gripper=franka_hand gripper.executable_cfg.robot_ip=${ROBOT_IP}
else
    ...
    $LAUNCH_GRIPPER_CMD gripper=franka_hand gripper.executable_cfg.robot_ip=${ROBOT_IP} use_real_time=false
fi
