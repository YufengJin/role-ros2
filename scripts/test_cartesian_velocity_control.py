#!/usr/bin/env python3
"""
Test script for Cartesian Velocity Control mode with arm and gripper.

This script:
1. Switches to cartesian_velocity_example_controller
...
class CartesianVelocityControlTester(Node):
    def __init__(self):
        super().__init__('cartesian_velocity_control_tester')
    ...
    def run_test(self):
        """Run the cartesian velocity control test"""
        self.get_logger().info('=' * 60)
        self.get_logger().info('Starting Cartesian Velocity Control Test')
        self.get_logger().info('=' * 60)
        ...
        current_arm_controllers = ['fr3_arm_controller', 'cartesian_pose_example_controller',
                                   'joint_position_example_controller', 'joint_velocity_example_controller']
        deactivate = [c for c in current_arm_controllers if c in active]
        if deactivate:
            self.switch_controllers([], deactivate, strictness=2)  # BEST_EFFORT
            time.sleep(1.0)
        
        # Step 3: Activate cartesian_velocity_example_controller
        self.get_logger().info('\n[Step 3] Activating cartesian_velocity_example_controller...')
        ...
        if not success:
            self.get_logger().error('Failed to activate cartesian_velocity_example_controller')
        ...
        self.get_logger().info('\n[Step 4] Running cartesian velocity controller for 5 seconds...')
        self.get_logger().info('WARNING: Robot will move with cartesian velocity commands!')
        ...
        self.get_logger().info('\n' + '=' * 60)
        self.get_logger().info('Cartesian Velocity Control Test Completed Successfully!')
        self.get_logger().info('=' * 60)
        return True
...
def main(args=None):
    ...
    tester = CartesianVelocityControlTester()
    ...
    try:
        ...
        tester.switch_controllers([], ['cartesian_velocity_example_controller'], strictness=2)
        sys.exit(1)
