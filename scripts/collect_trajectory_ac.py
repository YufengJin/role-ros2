#!/usr/bin/env python3
"""
Collect Trajectory with Action Chunking - Python script for collecting robot trajectories using VR controller.

This script extends collect_trajectory.py with action chunking functionality:
- Execute actions for n steps (action_steps)
- Sleep (zero action) for m steps (sleep_steps)
- Repeat this pattern throughout the trajectory

This simulates action chunking behavior where the policy outputs actions for a chunk of steps,
then holds (zero action) for a period.

Usage:
    # With action chunking (4 action steps, 2 sleep steps)
    python3 collect_trajectory_ac.py --task pick_and_place --action-steps 4 --sleep-steps 2
    
    # Full options
    python3 collect_trajectory_ac.py \
        --task pick_and_place \
        --action-steps 4 \
        --sleep-steps 2 \
        --save-images \
...
class CollectTrajectoryAC:
    """
    Python class for collecting robot trajectories using VR controller with Action Chunking.
    
    This class extends CollectTrajectory with action chunking:
    - Execute actions for action_steps steps
    - Sleep (zero action) for sleep_steps steps
    - Repeat this pattern
    ...
    def __init__(self, args):
        """Initialize CollectTrajectoryAC with parsed arguments."""
        # Parse arguments
        ...
        self.sleep_steps = args.sleep_steps
        
        # Action chunking state
        self._action_chunk_counter = 0  # Current step within current chunk phase
        self._is_action_phase = True  # True = action phase, False = sleep phase
        ...
        if not rclpy.ok():
            rclpy.init()
        self._node = Node('collect_trajectory_ac_node')
        
        # Initialize RobotEnv
        self._print("=" * 70)
        self._print("🚀 Collect Trajectory with Action Chunking - Starting Initialization")
        self._print("=" * 70)
        ...
        self._print(f"   • Action chunking: {self.action_steps} action steps, {self.sleep_steps} sleep steps")
        self._print("-" * 70)
    ...
    def _start_new_trajectory(self):
        ...
        # Reset action chunking state
        self._action_chunk_counter = 0
        self._is_action_phase = True  # Start with action phase
        ...
        if self.save_trajectory and self.save_folder:
            ...
            # Add action chunking metadata
            metadata["action_chunking"] = True
            metadata["action_steps"] = self.action_steps
            metadata["sleep_steps"] = self.sleep_steps
            ...
            self._print(f"📁 Recording trajectory #{self._traj_count} (Action Chunking: {self.action_steps}/{self.sleep_steps})...")
        else:
            self._traj_writer = None
            self._current_traj_filepath = None
            self._current_traj_dir = None
            self._current_time_str = None
            self._print(f"🎮 Trajectory #{self._traj_count} (teleoperation only, Action Chunking: {self.action_steps}/{self.sleep_steps})")
    ...
    def _control_loop(self):
        """Main control loop with action chunking."""
        ...
        try:
            ...
            if self.enable_viz and self.gui:
                phase_str = "ACTION" if self._is_action_phase else "SLEEP"
                chunk_info = f"[{self._action_chunk_counter + 1}/{self.action_steps if self._is_action_phase else self.sleep_steps}]"
                self.gui.set_status(f"Recording - {phase_str} {chunk_info}")
            ...
            # Action Chunking Logic: Determine if we should execute action or sleep (zero action)
            if self._is_action_phase:
                # Action phase: execute the action
                execute_action = True
            else:
                # Sleep phase: send zero action (hold current state)
            ...
            if skip_action or not execute_action:
                # Skip action (controller not enabled) or sleep phase (zero action)
                action_info = self.env.create_action_dict(np.zeros_like(action))
            else:
                # Execute action (action phase)
                action_info = self.env.step(action)
            action_info.update(controller_action_info)
            
            # Add action chunking metadata to action_info
            action_info["action_chunking"] = {
                "is_action_phase": self._is_action_phase,
                "chunk_counter": self._action_chunk_counter,
            ...
            # Update action chunking state
            self._action_chunk_counter += 1
            
            if self._is_action_phase:
                # Check if we've completed action phase
                if self._action_chunk_counter >= self.action_steps:
                    # Switch to sleep phase
                    self._is_action_phase = False
                    self._action_chunk_counter = 0
            else:
                # Check if we've completed sleep phase
                if self._action_chunk_counter >= self.sleep_steps:
            ...
            if self._num_steps % 50 == 0 and self._num_steps > 0:
                ...
                phase_str = "ACTION" if self._is_action_phase else "SLEEP"
                self._print(f"Step {self._num_steps}: {movement_status} | Phase: {phase_str} [{self._action_chunk_counter}/{self.action_steps if self._is_action_phase else self.sleep_steps}]")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Collect robot trajectories using VR controller with Action Chunking',
...
def main():
    ...
    try:
        collector = CollectTrajectoryAC(args)
